...
```bash
sudo apt-get install build-essentials cmake libglew-dev libglfw3-dev libglm-dev
```
...
* Download and install [TDM64](http://tdm-gcc.tdragon.net), this is the latest GCC compiler for Windows in a nice installer.
* Run CLion and select TDM64 as the __toolchain__ when prompted (default is C:/TDM64..)
* Setup the rest of the settings as you see fit.
* Open this directory and point the IDE towards the `CMakeLists.txt` file.

If CLion is too resource intensive for your system you can also try [QTCreator IDE](http://www.qt.io/ide/) that should work out of the box with TDM64 and CMake.
