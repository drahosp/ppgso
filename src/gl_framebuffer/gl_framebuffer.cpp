...
#include "gl_framebuffer_texture_vert.h"
#include "gl_framebuffer_texture_frag.h"
...
private:
  ...
  Shader program1{gl_framebuffer_texture_vert, gl_framebuffer_texture_frag};
  Mesh object1{program1, "sphere.obj"};
  Texture texture1{"sphere.rgb", 256, 256};

  // Objects to render the framebuffer on to
  Shader program2{gl_framebuffer_vert, gl_framebuffer_frag};
  Mesh object2{program2, "quad.obj"};
  Texture framebuffer{SIZE, SIZE};

  GLuint fbo, rbo;
public:
  FramebufferWindow(string title, unsigned int width, unsigned int height) : Window{title, width, height} {
...
int main() {
  ...
  auto window = FramebufferWindow{"gl_framebuffer", SIZE, SIZE};
