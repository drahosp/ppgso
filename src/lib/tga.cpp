...
TGAColor::TGAColor(unsigned char v) : bgra(), bytespp(1) {
  for (int i = 0; i < 4; i++)
    bgra[i] = 0;
  bgra[0] = v;
}
...
unsigned char &TGAColor::operator[](const int i) { return bgra[i]; }
...
TGAColor TGAColor::operator*(float intensity) const {
  ...
  for (int i = 0; i < 4; i++)
    res.bgra[i] = (unsigned char) ((float) bgra[i] * intensity);
  return res;
}
