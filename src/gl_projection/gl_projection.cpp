...
private:
  ...
  Shader program{gl_projection_vert, gl_projection_frag};
...
public:
  ProjectionWindow(string title, unsigned int width, unsigned int height) : Window{title, width, height} {
...
int main() {
  ...
  auto window = ProjectionWindow{"gl_projection", SIZE, SIZE};
